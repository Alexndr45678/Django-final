...
from hw2_app.models import Customer, Order, Product
from datetime import date, timedelta
from django.core.files.storage import FileSystemStorage
from hw2_app.forms import ProductFormWidget
...
class OrderedProducts(View):
    def get(self, request, customer_id):
        ...
        last_week_orders = Order.objects.filter(
            customer_id=customer_id, created_at__gt=filter_last_week
        ).order_by("-created_at")
        last_month_orders = Order.objects.filter(
            customer_id=customer_id, created_at__gt=filter_last_month
        ).order_by("-created_at")
        last_year_orders = Order.objects.filter(
            customer_id=customer_id, created_at__gt=filter_last_year
        ).order_by("-created_at")
        ...
        return render(request, "hw2_app/customer_orders.html", context)
...
class CreateProduct(View):
    def get(self, request):
        ...
        return render(request, "hw2_app/create_product.html", {"form": form})
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            product = Product(
                name=name, description=description, price=price, qty=qty, img=image_name
            )
            product.save()
...
class ReadProduct(View):
    def get(self, request, product_id):
        ...
        return render(
            request,
            "hw2_app/read_product.html",
            context,
            status=200 if product else 404,
        )


class Index(View):
    def get(self, request):
        return render(request, "hw2_app/index.html")
